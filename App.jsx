...
// 백엔드 이벤트(스네이크케이스 아님, 그냥 필드명 동일)를 프론트에서 쓰던 모양으로 변환
function normalizeEvent(e) {
  return { ...e, read: e.is_read ?? e.read ?? false };
}

export default function App() {
  // 로그인 정보는 새로고침해도 유지되도록 localStorage에 저장
  ...
  // 로그인되어 있으면: 백엔드에서 이벤트/설정 불러오고, 웹소켓으로 실시간 갱신 구독
  useEffect(() => {
    if (!auth) return;
    ...
    const socket = connectStatusSocket((msg) => {
      if (msg.kind === 'event') {
        setEvents(prev => {
          if (prev.some(e => e.id === msg.event.id)) return prev; // 중복 방지
  ...
  function handleLogin(authInfo) {
    ...
    try {
      ...
      // localStorage를 못 쓰는 환경이면 그냥 이번 세션만 로그인 유지됨
    }
  }
  ...
  function handleLogout() {
    ...
    setAuth(null);
    try { localStorage.removeItem('robotSafetyAuth'); } catch { /* 무시 */ }
  }

  function addEvent(type, title, detail, duration) {
    // 화면에는 즉시 반영(낙관적 업데이트)하고, 백엔드에도 저장 요청을 보냄.
    // 백엔드가 실제로 만든 이벤트는 웹소켓으로 다시 돌아오는데,
    // 그때는 id가 달라서 위 소켓 핸들러에서 중복 검사로 걸러지진 않지만
    // 지금 규모(관리자 1명)에서는 큰 문제가 안 되어 단순하게 유지합니다.
  ...
  // 구역/임계값 설정이 바뀌면 화면에 바로 반영 + 백엔드에도 저장
      ...
      <div className="body-row">
        ...
        <div className="main">
          <Routes>
            ...
            <Route path="/zone-settings" element={<ZoneSettings settings={settings} onChange={handleSettingsChange} />} />
